...
if [[ -s "$SRA.bam.bai" ]]; then
  ...
  aws s3 cp $SRA.bam.bai $S3_OUT/bai/
fi
...
if [[ -s "$SRA.summary" ]]; then
  echo "  ...$SRA.summary detected. Uploading."
  aws s3 cp $SRA.summary $S3_OUT/summary/
fi

if [[ -s "$SRA.th" ]]; then
  echo "  ...$SRA.th detected. Uploading."
  aws s3 cp $SRA.th $S3_OUT/th/
fi   
